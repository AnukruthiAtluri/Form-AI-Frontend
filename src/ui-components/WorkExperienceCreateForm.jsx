...
import { createWorkExperience } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function WorkExperienceCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            workExperienceUserProfileId: modelFields?.UserProfile?.id,
          };
          await API.graphql({
            query: createWorkExperience.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFieldsToSave,
              },
      ...
      {...getOverrideProps(overrides, "WorkExperienceCreateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
