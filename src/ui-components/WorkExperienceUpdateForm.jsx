...
import { getWorkExperience, listUserProfiles } from "../graphql/queries";
import { updateWorkExperience } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function WorkExperienceUpdateForm(props) {
  const {
    id: idProp,
    workExperience: workExperienceModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = workExperienceRecord
      ? { ...initialValues, ...workExperienceRecord, UserProfile }
      : initialValues;
  ...
  const [workExperienceRecord, setWorkExperienceRecord] = React.useState(
    workExperienceModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getWorkExperience.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
      ...
      setWorkExperienceRecord(record);
    };
    queryData();
  }, [idProp, workExperienceModelProp]);
  React.useEffect(resetStateValues, [workExperienceRecord, UserProfile]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            workExperienceUserProfileId: modelFields?.UserProfile?.id ?? null,
          };
          await API.graphql({
            query: updateWorkExperience.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "WorkExperienceUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || workExperienceModelProp)}
