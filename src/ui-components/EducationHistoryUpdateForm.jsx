...
import { getEducationHistory, listUserProfiles } from "../graphql/queries";
import { updateEducationHistory } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function EducationHistoryUpdateForm(props) {
  const {
    id: idProp,
    educationHistory: educationHistoryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = educationHistoryRecord
      ? { ...initialValues, ...educationHistoryRecord, UserProfile }
      : initialValues;
  ...
  const [educationHistoryRecord, setEducationHistoryRecord] = React.useState(
    educationHistoryModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getEducationHistory.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEducationHistory
        : educationHistoryModelProp;
      const UserProfileRecord = record ? await record.UserProfile : undefined;
      setUserProfile(UserProfileRecord);
      setEducationHistoryRecord(record);
    };
    queryData();
  }, [idProp, educationHistoryModelProp]);
  React.useEffect(resetStateValues, [educationHistoryRecord, UserProfile]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            educationHistoryUserProfileId: modelFields?.UserProfile?.id ?? null,
          };
          await API.graphql({
            query: updateEducationHistory.replaceAll("__typename", ""),
            variables: {
              input: {
                id: educationHistoryRecord.id,
                ...modelFieldsToSave,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "EducationHistoryUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || educationHistoryModelProp)}
